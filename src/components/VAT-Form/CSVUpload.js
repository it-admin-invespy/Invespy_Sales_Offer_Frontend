        ...
        const data = csv
          ...
          .map((line) => {
            ...
            return {
              ...
              vatPrice: parseFloat(values[6]?.trim()) + (parseFloat(values[6]?.trim()) * 5) / 100,
            };
          })
          .sort(sortUnits);
          ...
          <table className="w-full border border-gray-300 text-sm">
            ...
            <tbody>
              {csvData.map((row, index) => {
                ...
                const defaultVatPrice = parseNumericValue(row.vatPrice);
                  ...
                  <tr key={index} className={isSelected ? "bg-blue-50" : ""}>
                    ...
                    <td className="border border-gray-300 px-2 py-1 text-right">
                      <input
                        type="hidden"
                        {...register(`${name}.${index}.price`)}
                        defaultValue={defaultVatPrice}
                      />
                      {formatCurrency(row.vatPrice)}
                    </td>
                    <td className="border border-gray-300 px-2 py-1">
